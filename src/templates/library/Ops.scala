  ...
  def emitOp(o: Rep[DSLOp], stream: PrintWriter, indent: Int = 0) {
    Impls(o) match {
      ...
      case filter:Filter =>
        ...
        emitWithIndent("val e = " + makeOpMethodName(dc.apply) + "(" + o.args.apply(filter.argIndex).name + ", i)", stream, indent)
  ...
  def emitClass(opsGrp: DSLOps, stream: PrintWriter) {
    if (grpIsTpe(opsGrp.grp)) {
      ...
      d.foreach { data => 
        ...
        for (o <- unique(opsGrp.ops) if o.style == infix && getHkTpe(o.args.apply(0).tpe) == tpe) {       
    ...
    for (o <- unique(opsGrp.ops)) {       
      ...
      o.style match {
        ...
        case `infix` if grpIsTpe(opsGrp.grp) => 
