...
trait SmallMapOpsExp extends EffectExp {
  this: DeliteOpsExp => 
  ...
  case class MapRemove[K:Manifest, V:Manifest](m: Exp[SmallMap[K,V]], key: Exp[K]) extends SmallMapDef[K,V,Any] 
  ...
  override def mirror[A:Manifest](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = ( e match {
    ...
    case e@MapRemove(m,k) => map_remove(f(m),f(k))(e.mK,e.mV,pos)
    ...
    case Reflect(e@MapRemove(m,k), u, es) => reflectMirrored(Reflect(MapRemove(f(m),f(k))(e.mK,e.mV), mapOver(f,u), f(es)))(mtype(manifest[A]))
...
trait ScalaGenSmallMapOps /* extends ScalaGenDeliteMapOps */ extends ScalaGenPrimitiveOps with ScalaGenObjectOps {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    ...
    case MapRemove(m,k) => emitValDef(sym, "" + quote(m) + ".remove(" + quote(k) + ")")
